...
{
    public sealed partial class HueLightControl : UserControl
    {
        public HueLightControlViewModel viewModel { get; set; } = new HueLightControlViewModel();
        public HueLightControl()
        {
        ...
        public static readonly DependencyProperty ApplianceProperty =
          DependencyProperty.Register(nameof(Appliance), typeof(IAppliance),
            typeof(HueLightControl), new PropertyMetadata(null, new PropertyChangedCallback(OnApplianceChanged)));

        private static void OnApplianceChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            var cc = d as HueLightControl;
            var appliance = (IAppliance)e.NewValue;
            if (appliance.ApplianceType == "Dimmable light")
            {
